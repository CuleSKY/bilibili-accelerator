...
| 版本 | 主要变化 |
| --- | --- |
| v0.4.0 | 后台播放修复：切到别的标签页几秒后视频就卡住（Safari 最明显）。原因是加速器把 B 站自家的海外镜像改写去了国内 CDN，缓冲一直太薄——现在这些镜像不再动。另外不再克隆媒体响应，切标签页也不会误触发卡顿恢复。备选服务器列表也补上了海外镜像——之前只有国内那几台，海外用户卡一次就会被切到慢得多的线路上；现在所有备选都会实测排序，卡顿切换也改成挨个试，不再在同样两台之间来回跳。排序改成看实际下载速度，不再看服务器应答多快——同一台的应答时间前后能差十倍，运气不好测一次就会被锁在慢线路上好几个小时。「总是切换服务器」也不会再把你从 B 站自家的海外镜像上挪走了 |
